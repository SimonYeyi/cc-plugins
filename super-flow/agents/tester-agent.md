...
4. **dispatch** test-reviewer 进行用例评审，传递SPEC.md、逻辑测试用例文档、人工测试用例文档
...
2. **判断**：
   - **通过** → 进入阶段四
   - **有意见，count < 5** → 修复/反驳评审意见，进入阶段二
   - **有意见，count = 5** → 汇总分歧上报主控决断
   - **count = -1（主控决断）** → 必须遵守，执行决断，更新测试文档，进入阶段四

### 阶段一（开发Agent修复反馈输入）
收到开发Agent修复反馈后，进入阶段二
...
### 阶段三：评审
1. **dispatch** test-reviewer 进行测试评审，传递SPEC.md、逻辑测试用例文档、人工测试用例文档、单元测试代码
