...
### 阶段一（SPEC.md与代码实现输入）：测试用例文档
1. **读取** SPEC.md，理解验收标准
2. **生成** 逻辑测试用例文档（TC-XXX）
3. **生成** 人工测试用例文档（MC-XXX）
4. **dispatch** test-reviewer 进行阶段一评审，传递SPEC.md、逻辑测试用例文档、人工测试用例文档
5. **评审循环**：根据意见补充/修复测试用例，最多5次
6. **评审通过后**

### 阶段一/阶段二（评审意见输入）：处理评审结果
**入口**：主控转发的评审结果（附带阶段标识）
1. **理解** 评审结果类型、count和阶段
2. **判断**：
   - **通过** → 确认评审通过，通知主控继续
   - **有意见，count < 5** → 修复/反驳评审意见
   - **有意见，count = 5** → 汇总分歧上报主控裁断
   - **count = 6（主控裁决）** → 必须遵守，执行裁决，更新测试文档，上报评审通过
...
### 阶段二（开发Agent修复反馈输入）：继续测试
收到开发Agent修复反馈后，重新执行测试

### 阶段二：评审
1. **dispatch** test-reviewer 进行阶段二评审，传递SPEC.md、逻辑测试用例文档、人工测试用例文档、测试报告
2. **评审循环**：根据意见修复，最多5次
3. **评审通过后**
...
- 测试失败时：测试代码错误打回测试Agent修复，功能不通过打回开发Agent
- 有效问题则修复，无理则反馈理由
- 最多5次内循环，5次后仍分歧升级主控裁断
