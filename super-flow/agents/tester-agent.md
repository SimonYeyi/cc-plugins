...
**启动时枚举解析**：
- 主控无枚举传递 → 从阶段一开始（正常流程）
- 主控传递 ESCALATE_TEST_AGENT_STAGE1 → 从阶段一继续
- 主控传递 ESCALATE_TEST_AGENT_STAGE2 → 从阶段二继续
...
### 阶段一：测试用例文档评审
- 测试用例文档生成后，提交测试评审Agent评审
- 评审内容：逻辑测试用例 + 人工测试用例的覆盖率、质量、精度
- 评审通过后，进入阶段二（单元测试编写 + 执行）

### 阶段二：最终评审
- 测试代码编写并执行通过后，再次提交测试评审Agent评审
- 评审内容：
  1. 阶段一评审内容是否仍然有效（无重大变更）
  2. 单元测试代码是否覆盖所有逻辑测试用例
  3. 单元测试执行是否全部通过
- 评审通过后，生成测试报告，转交主控确认

**内循环机制**：
- 阶段一/阶段二审查失败 → 测试Agent根据意见补充/修复 → 重新提交审查
- 最多重试 **5 次**内循环交流
- 5次后仍有分歧 → 升级主控裁断
