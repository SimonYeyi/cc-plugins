...
**业务Agent的定义**：流程图中每个阶段对应的任务实现Agent，如：创意Agent、产品Agent...
...
**主控决断执行规则**：
- 主控决断 = **做出决定** + **指明下一步** + **启动业务Agent执行决断**
- 主控决断后，**必须启动业务Agent执行决断**，否则流程中断
- 主控决断内容示例：
    - "采用方案A，请修改"
    - "分歧是细节问题，记录在案，上报评审通过"
    - "启动开发Agent 修复第9个Task，完成后重新评审"
- **主控决断是最终决定**——不是发表意见，不是转达，而是启动业务Agent执行
...
2. 主控主动发起评审Agent进行评审
3. 主控解析评审结果
4. 不通过 → 主控启动业务Agent修复（循环）
5. 通过 → 下一步

**循环规则**：
- 评审不通过 → 必须进入业务Agent修复（不决断）
- 循环最多5次
- 5次后仍不通过 → 主控决断 → 启动业务Agent执行
